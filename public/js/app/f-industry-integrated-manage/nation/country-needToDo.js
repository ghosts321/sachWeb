...
$(function () {
    ...
    try {
        ...
        for (var i = 0; i < roleClassArr.length; i++) {
            if (roleClassArr[i] == '4' || roleClassArr[i] == '5') { //处长处员 区分普通处理和专家待办
                cmx.g.needtodocondition = [201, 202, 203, 204, 206, 207, 208, 209, 213, 214, 215, 216, 223, 224, 225, 226, 227, 303, 304, 305];
    ...
    $('#search-export').on('click', function () {
        var format =
            ...
            // 'publishType=' + (($('#publishTypeName').val() == '-1') ? '' : $('#publishTypeName').val()) + '&' +
        ...
        console.log(format);
        window.open(api_ea + '/businesspplan/exportExcelList?token=' + getData('token') + '&listType=1&' + format);
    });
});
var needFunc = {};
needFunc.pageNum = 1;
...
function changeNeedTodoState(type) {
    globelNeedType = type;
    if (type == 0) {
        cmx.g.needtodocondition = [201, 202, 203, 204, 206, 207, 208, 209, 213, 214, 215, 216, 223, 224, 225, 226, 227, 303, 304, 305, 497, 498, 877, 878, 495, 496, 987, 499, 773, 776, 777, 999];
        $('#search-export').off('click');
        $('#search-export').on('click', function () {
            var format =
                'proFileTitle=' + $('#proFileTitle').val() + '&' +
        ...
        $('#search-export').on('click', function () {
            var format =
                ...
                'proSendTimeEnd=' + $('#proSendTimeEnd').val().replaceAll('年', '-').replaceAll('月', '-').replaceAll('日', '')
            console.log(format);
            window.open(api_ea + '/businesspplan/exportExcelList?token=' + getData('token') + '&listType=4&' + format);
        });
    }
    needFunc.pageNum = 1;
    $('#select-applynum-input').val('');
