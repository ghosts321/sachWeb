...
$(document).ready(function () {
        ...
        new cmx.process()
            ...
            .turn(function (prevModelData, send, abort) {
                if (!IsNull(prevModelData) && prevModelData.state == '200' && !IsEmpty(prevModelData.data) && prevModelData.data != 'null') {
                    ...
                    if (!IsEmpty(data.eaPubThreenotice)) {
    ...
    var paramStr = '?from=iframe&check=wss-xsjc&status=' + _status + '&applyId=' + _applyId + '&projectNum=' + _projectNum;
    ...
    if (!IsEmpty(GetUrlParamString('applyId'))) {
        var definstid = "";
        new cmx.process()
            .turn('callajax', { //获取默认机构id
                url: api_ea + '/eaPubAppriteminfo/getDataByItemId',
                data: JSON.stringify({
                    token: getData('token'),
                    projectNum: _projectNum
                }),
            ...
            .turn(function (prevModelData, send, abort) {
                if (prevModelData.state == '200' && !IsEmpty(prevModelData.data) && prevModelData.data != 'null') {
                    definstid = prevModelData.data;
                } else {
                    abort('获取默认机构失败');
                }
                send.go();
            ...
            .turn('callajax', { //获取司列表
                url: api_aa + '/inst/aaSachinst/getsiListByInstid',
                data: JSON.stringify({
                    token: getData('token'), //类型：String  必有字段  备注：无
                    definstid: '"' + definstid + '"'
                }),
            ...
            .turn(function (prevModelData, send, abort) {
                if (!IsNull(prevModelData) && prevModelData.state == '200' && !IsEmpty(prevModelData.data) && prevModelData.data != 'null') {
                    ...
                    for (var i = 0; i < param.length; i++) {
                        if (_projectNum == '56011' && param[i].instId != 'GJ04000000')
                            continue;
                        html = html + [
                            '<div class="radio-custom radio-default radio-inline">',
                            '<input type="radio" id="cmx-department-' + i + '" name="department" value="' + param[i].instId + '" ' + ((param[i].instId == definstid) ? "checked" : "") + '>',
                            '<label for="cmx-department-' + i + '">' + param[i].instName + '</label>',
                            '</div>'
                        ].join('');
                    }
                    $('#cmx-department-list').html(html);
                }
                send.go();
            })
            .start();
    ...
    $("#cmx-button-confirm").on('click', function () {
        ...
        if ($("#cmx-fenpai").hasClass("active")) {
            if (IsEmpty($('#cmx-department-list').find('input[type="radio"]:checked').val())) {
                showAlert({
                    type: 'error',
                    content: '请选择分派人员'
                });
            ...
            new cmx.process()
                .turn('callajax', {
                    ...
                    data: JSON.stringify({
                        ...
                        chooseSi: $('#cmx-department-list').find('input[type="radio"]:checked').val() //$('#cmx-department-list').find('input[type="radio"]:checked').val()
                    }),
                    type: 'POST'
                })
                .turn(function (prevModelData, send, abort) {
                ...
                //         trsFormData: [{
                //             applyId: _applyId,
                //             projectNum: "56012", //TODO 这里还是固定值，和办理记录相关，下方的success的内容或许也需要调整修改
                //             receiverId: $('#cmx-department-list').find('input[type="radio"]:checked').val(),
                ...
                //         if (result.state == '200') {
                //             setTimeout(function () {
                //                 window.location.href = '/app/f-gover-approval/country/country-needToDo.html?nowid='+GetUrlParamString('nowid');
                //             }, 1000);
                //         }
                //     }
                // })
            ...
            new cmx.process()
                .turn('callajax', {
                    ...
                    data: JSON.stringify({
                        ...
                        chooseSi: "" //$('#cmx-department-list').find('input[type="radio"]:checked').val()
                    }),
                    type: 'POST'
                })
                .turn(function (prevModelData, send, abort) {
