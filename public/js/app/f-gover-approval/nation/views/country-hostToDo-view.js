...
var donestatus = new Array('210', '504', '302', '902', '999', '994', '886', '868', '992', '882', '773');
...
cmx.route.view({
    ...
    resolve: function (result) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (donestatus.indexOf(data[i].status) > -1) {
                workDaysColor = 'badge-default';
            }
            tbody_html = ['<tr>',
                ...
                '<input type="checkbox" id="inputUnchecked' + data[i].applyId + '" ' + ((cmx.g.reminderArray.get(data[i].applyId)) ? 'checked' : '') + '>',
                '<label for="inputUnchecked' + data[i].applyId + '"></label>',
                '</div>',
                '</td>',
                ...
                '<td><span class="badge badge-radius ' + workDaysColor + '">' + ((IsEmpty(data[i].workDay) || donestatus.indexOf(data[i].status) > -1 || data[i].applyStepFlag == '3') ? '无' : data[i].workDay + '天') + '</span></td>',
        ...
        $("#cmx-htd-tbody tr").each(function (index) {
            ...
            $(this).bind('click', function () {
                ...
                putData('_chopn', haveFunc.pageNum);
                if (data[index].applyStepFlag != 3) {
                    ...
                    window.location.href = '/app/f-gover-approval/nation/qianbao.html?type=8&from=undefied&status=' + data[index].status + '&applyId=' + data[index].applyId + '&projectNum=' + data[index].projectNum + '&nowid=' + GetUrlParamString('nowid');
                }
                //window.location.href = '/app/f-gover-approval/applyinfo.html?from=undefied&status=' + data[index].status + '&applyId=' + data[index].applyId + '&projectNum=' + data[index].projectNum + '&nowid=' + GetUrlParamString('nowid');
            });
        });
