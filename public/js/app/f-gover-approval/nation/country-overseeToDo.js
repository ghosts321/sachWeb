            ...
            if (getData('condition_str')) {
                try {
                    if (GetUrlParamString('back') == '-1') {
                        ...
                        $('#unitName').val(json_con.protectUnitName);
...
haveFunc.pageNum = 1;
if (GetUrlParamString('back') == '-1') {
    if (!IsEmpty(getData('_copn'))) {
        haveFunc.pageNum = getData('_copn');
    }
}
            ...
            send.tomodel({
                data: JSON.stringify({
                    ...
                    tdlFormData: [{
                        ...
                        receivedStartTime: $('#proSendTimeStart').val().replaceAll('年', '-').replaceAll('月', '-').replaceAll('日', ''),
                        receivedEndTime: $('#proSendTimeEnd').val().replaceAll('年', '-').replaceAll('月', '-').replaceAll('日', ''),
