...
$(document).ready(function () {
    if (!IsEmpty(GetUrlParamString('applyId')) && !IsEmpty(GetUrlParamString('projectNum')) && !IsEmpty(GetUrlParamString('status'))) {
        ...
        if (GetUrlParamString('type') && GetUrlParamString('type') == '0') {
            _turnFlag = true;
            $('#cmx-timeLimit').hide();
            $('#cmx-tabsForwardingText').hide();
            $('#cmx-branchTabs').hide();
            $('#cmx-brife-table-info').hide();
            $('[href="#processTabTwo"]').parent().hide();
            $('[href="#processTabThree"]').parent().hide();
        }
