...
cmx.route.model({
    ...
    handle: function (parameter, prevModelData, send, abort) {
        ...
        if (param.state == '200') {
            ...
            setTimeout(function () {
                window.location.href = public_url + 'app/f-gover-approval/56020-2/7shenpi.html?nowid=' + GetUrlParamString('nowid');
            }, 1000)
        } else {
            showAlert({
                type: 'error',
    ...
    handle: function (parameter, prevModelData, send, abort) {
        ...
        if (param.state == '200') {
            if (param.data > 0) {
                showAlert({
                    ...
                    callback: function (_state) {
                        if (_state == 'yes') {
                            new cmx.process()
                                .turn('callajax', {
                                    url: api_ea + '/eaScrRelicInfo/TempEntersaveApprove',
                                    data: JSON.stringify({
                                        token: getData('token'),
                                        applyId: parameter
                                    })
                ...
                getApplyList(2, '/eaScrApplyInfo/getApproveList', 8);
            }
        ...
        $(".editRelic").click(function () {
            ...
            $("#cmx-relic-modal").load(editRelicInfoModal, function () {
                ...
                $('#cmx-editrelicinfo').on('shown.bs.modal', function () {
                    ...
                    // $('#cmx-special-P0028').parent().css('display','none');
                    $('.isAllowName').remove();
