...
cmx.route.model({
    ...
    handle: function (parameter, prevModelData, send, abort) {
        ...
        if (param.state == '200') {
            ...
            setTimeout(function () {
                window.location.href = public_url + 'app/f-gover-approval/56020-2/5dengji.html?nowid=' + GetUrlParamString('nowid');
            }, 1000)
        } else {
            showAlert({
                type: 'error',
            ...
            showAlert({
                ...
                callback: function (_state) { //仅type为confirm下有效
                    ...
                    if (_state == 'yes') {
                        ...
                        new cmx.process()
                            .turn('callajax', {
                                url: api_ea + '/eaScrApplyInfo/completeTempEnterRegister',
                                data: JSON.stringify({
                                    token: getData('token'),
                                    applyId: parameter
                                })
