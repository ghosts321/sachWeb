    ...
    resolve: function (param) {
        ...
        if (data.length > 0) {
            for (var i = 0; i < data.length; i++) {
                ...
                var regStatus = data[i].regStatus == 1 ? '海关文件' : '上传附件';
                ...
                '<button func-id="0102390300000000"  data-type="' + applyClassName + '" class="btn btn-primary btn-xs exit-application-update" data-id="' + applyId + '">修改</button><button func-id="0101030102040200" data-id="' + applyId + '" data-type="' + applyClassName + '"  class="btn btn-primary btn-xs cmx-update">' + regStatus + '</button><button func-id="0101030102040300" data-id="' + applyId + '" style="margin:5px;" class="btn btn-primary btn-xs cmx-accept">提交</button><button func-id="0101030102040400" data-id="' + applyId + '"  style="margin:5px;" class="btn btn-primary btn-xs cmx-refuse">退回</button>',
        ...
        $('.cmx-update').on('click', function () {
            ...
            $("#cmx-applicationForExhibition-modal").load(uploadModal, function () {
                ...
                $('#cmx-applyForExhibition-info').on('shown.bs.modal', function () {
                    ...
                    $("#applyForExhibitionSave").on('click', function () {
                        var length = cmx.g.filesarray.get('157').length - 1;
                        var a = cmx.g.filesarray.get('157')[length];
                        if (IsEmpty(a)) {
                            showAlert({
                                type: 'info',
                                content: '文件不能为空!'
                            ...
                            new cmx.process().
                                turn('callajax', {
                                    url: api_ea + "/eaScrApplyInfo/uploadFile",
                                    data: JSON.stringify({
                                        token: getData("token"),
                                        applyId: applyId,
                                ...
                                .turn(function (prevModelData, noviewsend, abort) {
                                    if (prevModelData.state == '200') {
                                        ...
                                        new cmx.process().
                                            ...
                                            .start();
                                            getApplyList(2, '/eaScrApplyInfo/getCurrenList', 5);
                                    }
                                })
                                .start();
                        }
                    ...
                    $('#cmx-applyForExhibition-info').on('shown.bs.modal', function () {
                        ...
                        setTimeout(function () {
                            ...
                            $('#applyForExhibitionSave').on('click', (function () {
                                ...
                                var index = cmx.g.filesarray.get('157')[0];
                                if (IsEmpty(index)) {
                                    index = ''
                                }
                                new cmx.process()
...
cmx.route.view({
    ...
    resolve: function (param) {
        ...
        $("#cmx-1-CustPurpose").val(data.custPurpose).attr('disabled', 'true');
        ...
        $("#cmx-1-IDCardType").val(data.idcardType).attr('disabled', 'true');
