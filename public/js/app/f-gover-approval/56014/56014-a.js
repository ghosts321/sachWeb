 ...
 * @Last Modified time: 2018-05-21 16:51:44
 */
var get56014aNoNationProjectFormData = public_url + 'data/app/f-gover-approval/56014/56014-a.json';
var get56014aZipProjectFormData = public_url + 'data/app/f-gover-approval/56014/56014-a-zip.json';
...
$(document).ready(function () {
                    ...
                    if (prevModelData.state == '200') {
                        ...
                        for (var i = 0; i < planList.length; i++) {
                            html = html + [
                                ...
                                '<td>' + (i + 1) + '</td>',
                                '<td><button class="btn btn-warning btn-xs" onclick="deletePlanZip(event,this,\'' + planList[i].planId + '\');">移除</button></td>',
                                '<td>' + planList[i].unitType + '</td>',
                                '<td>' + planList[i].projectName + '</td>',
                                '<td>' + planList[i].protectUnitName + '</td>',
                                '<td>' + planList[i].projectType + '</td>',
                                '<td>' + planList[i].estimateAmount + '</td>',
                                '<td>' + planList[i].countryAmount + '</td>',
                                '</tr>',
                            ].join('');
                        }
    ...
    $('#cmx-old-build').on('click', function () {
        if (!IsEmpty(cmx.g.applyId)) {
            downloadThisFile('6C9A78D2-3B3F-43E8-A51A-B415768DD56E');
        }
    })
    $('#cmx-old-ruins').on('click', function () {
        if (!IsEmpty(cmx.g.applyId)) {
            downloadThisFile('EF809512-C30A-4567-8669-ABCDF22C8C4E');
        }
    })
    ...
    if (!IsEmpty(GetUrlParamString('applyId'))) {
        ...
        setTimeout(function () {
            window.location.href = "/app/f-gover-approval/apply-table.html?nowid="+GetUrlParamString('nowid');
        }, 2000);
    }
...
function showPlanDetail(planId, unitType) {
    new cmx.process()
        ...
        .turn(function (prevModelData, send, abort) {
            ...
            if (prevModelData.state == '200') {
                var data = prevModelData.data;
                if (unitType == '古遗址古墓葬类' || unitType == '古遗址' || unitType == '古墓葬') {
                    $("#showMuzangPlanDetailModal").off('show.bs.modal');
                    $("#showMuzangPlanDetailModal").on('show.bs.modal', function () {
                        var html = '';
                        for (var key in data) {
                            $('#showMuzangPlanDetailModal [key="' + key + '"]').text(data[key]);
                        }
                        var hisList = data.hisList;
                        for (var i = 0; i < hisList.length; i++) {
                            html = html + [
                                ...
                                '<td>' + hisList[i].begDate + '</td>',
                                '<td>' + hisList[i].endDate + '</td>',
                                '<td>' + hisList[i].subsidyAmount + '</td>',
                                '<td>' + hisList[i].usedAmount + '</td>',
                                '<td>' + hisList[i].unusedAmount + '</td>',
                                '</tr>',
                            ].join('');
                        }
                        $('#key-muzang-hisList').html(html);
                        $('#key-muzang-file').html('');
                        var fileList = data.fileList;
                        for (var i = 0; i < fileList.length; i++) {
                            ...
                            new cmx.process()
                                ...
                                .turn(function (result, send, abort) {
                                    ...
                                    if (!IsNull(result) && result.state == '200' && !IsEmpty(result.data) && result.data != 'null') {
                                        ...
                                        console.log(response)
                                        $('#key-muzang-file').append('<button class="btn btn-default btn-sm margin-right-10" onclick="downloadThisFile(\'' + _fileIndex + '\')">' + response + '</button>');
                                    }
                                })
                                .start();
                        }
                        $('#key-muzang-address').html(data.province + data.city + data.county + data.town + data.village);
                    });
                    $("#showMuzangPlanDetailModal").modal('show');
                } else {
