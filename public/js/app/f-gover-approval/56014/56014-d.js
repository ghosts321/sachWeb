 ...
 * @Last Modified time: 2018-06-20 14:59:35
 */

var _projectType = '2';
var _projectNum = '56014_d';
var get56014dNoNationProjectFormData = public_url + 'data/app/f-gover-approval/56014/56014-d.json';
$(document).ready(function () {
    ...
    CreateApplicationForm({
        "url": get56014dNoNationProjectFormData,
        "element": "#cmx-form"
    });
    ...
    $("#cmx-noNationProject-save").on("click", function () {
        ...
        if (!IsEmpty($("#cmx-i-41").val())) {
            if ($("#cmx-i-41").val().length > 40) {
                showAlert({
                    type: "info",
                    content: "批复文号可填最大长度为40"
                });
                return;
            }
        }
        if (!IsEmpty($("#cmx-i-42").val())) {
            if ($("#cmx-i-42").val().length > 1000) {
                showAlert({
                    type: "info",
                    content: "批复标题可填最大长度为1000"
                });
                return;
            }
        }
        ...
        new cmx.process()
            .turn('callajax', {
                url: api_ea + '/eaIchPphApply/saveprocessInstanceId',
                data: JSON.stringify({
    ...
    $("#cmx-noNationProject-send").on("click", function () {
        ...
        new cmx.process()
            .turn('callajax', {
                url: api_ea + '/eaIchPphApply/sendEaIchPphApply',
                data: JSON.stringify({
                    token: getData('token'), //类型：String  必有字段  备注：无
                    pphFormData: data,
                    roleId: 'RSJ0000006'
    ...
    if (!IsEmpty(GetUrlParamString('applyId'))) {
        ...
        new cmx.process()
            .turn('callajax', {
                url: api_ea + '/eaIchPphApply/getEntityByPKAndPNum',
                data: JSON.stringify({
                    token: getData('token'), //类型：String  必有字段  备注：无
