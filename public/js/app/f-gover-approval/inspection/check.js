 ...
 * @Last Modified time: 2018-07-11 15:30:43
 */
...
$(document).ready(function () {
    ...
    new cmx.process()
        ...
        .turn(function (prevModelData, send, abort) {
            ...
            if (!IsNull(prevModelData) && prevModelData.state == '200') {
                $('#provinceOpinion').val(prevModelData.data.checkOpinion);
    ...
    $('#cmx-button-return').on('click', function () {
        location.href = '/app/f-gover-approval/inspection/inspection-checkToDo.html?nowid=' + GetUrlParamString('nowid');
    });
    ...
    $('#cmx-options-save').on('click', function () {
        ...
        new cmx.process()
            .turn('callajax', {
                url: api_ea + '/eaPubApplyprecheck/saveEaPubApplyprecheck',
                data: JSON.stringify({
                    token: getData('token'), //类型：String  必有字段  备注：无
                    applyPrecheck: [{
                        checkId: _examId,
                        checkOpinion: $('#provinceOpinion').val(),
                        applyId: _applyId,
    ...
    $('#cmx-options-send').on('click', function () {
        ...
        new cmx.process()
            .turn('callajax', {
                url: api_ea + '/eaPubApplyprecheck/sendReviewOpinion',
                data: JSON.stringify({
                    token: getData('token'), //类型：String  必有字段  备注：无
                    applyPrecheck: [{
                        checkId: _examId,
    ...
    $('#cmx-options-back').on('click', function () {
        ...
        new cmx.process()
            .turn('callajax', {
                url: api_ea + '/eaPubApplyprecheck/refuseEaPubApplyprecheck',
                data: JSON.stringify({
                    token: getData('token'), //类型：String  必有字段  备注：无
                    applyPrecheck: [{
                        checkId: _examId,
