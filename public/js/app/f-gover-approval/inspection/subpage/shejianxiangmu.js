var getShejianxiangmuData = public_url + 'data/app/f-gover-approval/inspection/shejianxiangmu.json';
...
$(document).ready(function() {
    ...
    $("#cmx-noNationProject-save").on("click", function() {
        ...
        var ss = [{ //类型：Object  必有字段  备注：无
            ...
            registerNum: '',
            serialNum: '',
            item1: $("#cmx-i-2").val(), //建控要求
            item2: $("#cmx-i-3").val(), //相关资料图纸
            item3: $("#cmx-i-4").val(), //设计说明

            expertOpinion: $("#cmx-i-6").val(), //评审意见
            expertResults: $('#cmx-i-7 input[type="radio"]:checked').val(), //评审结论
            remark: $("#cmx-i-8").val(), //备注
        }];
        console.log(ss);
        new cmx.process()
            .turn('callajax', {
...
cmx.route.model({
    index: 'buildshejianxiangmuInput',
    handle: function(parameter, prevModelData, send, abort) {
        ...
        if (!IsNull(prevModelData) && prevModelData.state == '200' && !IsEmpty(prevModelData.data) && prevModelData.data != 'null') {
            ...
            $("#cmx-i-2").val(data.item1); //建控要求
            $("#cmx-i-3").val(data.item2); //相关资料图纸
            $("#cmx-i-4").val(data.item3); //设计说明

            $("#cmx-i-6").val(data.expertOpinion); //意见
            $("#cmx-i-8").val(data.remark); //备注
            $('#cmx-i-7 input[value="' + data.expertResults + '"]').attr("checked", true);
