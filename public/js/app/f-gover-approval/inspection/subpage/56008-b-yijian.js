var get56008byijianData = public_url + 'data/app/f-gover-approval/inspection/56008-b-yijian.json';
cmx.g.regist('examId', '');
$(document).ready(function() {
    ...
    $("#cmx-noNationProject-save").on("click", function() {
        ...
        var ss = [{ //类型：Object  必有字段  备注：无
            ...
            registerNum: '',
            serialNum: '',
            expertOpinion: $("#cmx-i-2").val(), //意见
            expertResults: $('#cmx-i-3 input[type="radio"]:checked').val(), //评审结论
        }];
        console.log(ss);
        new cmx.process()
            .turn('callajax', {
...
cmx.route.model({
    ...
    handle: function(parameter, prevModelData, send, abort) {
        ...
        if (!IsNull(prevModelData) && prevModelData.state == '200' && !IsEmpty(prevModelData.data) && prevModelData.data != 'null') {
            ...
            $("#cmx-i-2").val(data.expertOpinion); //意见
            $('#cmx-i-3 input[value="' + data.expertResults + '"').attr("checked", true);
