var getKaoguData = public_url + 'data/app/f-gover-approval/inspection/kaogu.json';
...
$(document).ready(function() {
    ...
    $("#cmx-noNationProject-save").on("click", function() {
        ...
        var ss = [{ //类型：Object  必有字段  备注：无
            ...
            item1: $("#cmx-i-2").val(), //价值评估
            item2: $("#cmx-i-3").val(), //现状评估
            item3: $("#cmx-i-4").val(), //保护区划
            item4: $("#cmx-i-5").val(), //保护措施
            item5: $("#cmx-i-6").val(), //专项规划
            item6: $("#cmx-i-7").val(), //规划图纸
            expertOpinion: $("#cmx-i-9").val(), //意见
            expertResults: $('#cmx-i-10 input[type="radio"]:checked').val(), //专家评审意见
            remark: $("#cmx-i-11").val(), //备注
        }];
        console.log(ss);
        new cmx.process()
            .turn('callajax', {
...
cmx.route.model({
    ...
    handle: function(parameter, prevModelData, send, abort) {
        ...
        if (!IsNull(prevModelData) && prevModelData.state == '200' && !IsEmpty(prevModelData.data) && prevModelData.data != 'null') {
            ...
            $("#cmx-i-2").val(data.item1);
            $("#cmx-i-3").val(data.item2);
            $("#cmx-i-4").val(data.item3);
            $("#cmx-i-5").val(data.item4);
            $("#cmx-i-6").val(data.item5);
            $("#cmx-i-7").val(data.item6);
            $("#cmx-i-9").val(data.expertOpinion);
            $('#cmx-i-10 input[value="' + data.expertResults + '"').attr("checked", true);
            $("#cmx-i-11").val(data.remark);
