var getGongchengfanganData = public_url + 'data/app/f-gover-approval/inspection/gongchengfangan.json';
...
$(document).ready(function() {
    ...
    $("#cmx-noNationProject-save").on("click", function() {
        ...
        var ss = [{ //类型：Object  必有字段  备注：无
            ...
            registerNum: '',
            serialNum: '',
            item1: $("#cmx-i-2").val(), //现状勘察现状照片及现状图
            item2: $("#cmx-i-3").val(), //保护原则
            item3: $("#cmx-i-4").val(), //工程性质、范围、规模定位
            item4: $("#cmx-i-5").val(), //保护措施、方案图纸
            expertOpinion: $("#cmx-i-7").val(), //意见
            expertResults: $('#cmx-i-8 input[type="radio"]:checked').val(), //专家评审意见
            remark: $("#cmx-i-9").val(), //备注
        }];
        console.log(ss);
        new cmx.process()
            .turn('callajax', {
    ...
    if (!IsEmpty(GetUrlParamString('examId'))) {
        ...
        new cmx.process()
            ...
            .turn('buildgongchengfanganInput', {
...
cmx.route.model({
    index: 'buildgongchengfanganInput',
    handle: function(parameter, prevModelData, send, abort) {
        ...
        if (!IsNull(prevModelData) && prevModelData.state == '200' && !IsEmpty(prevModelData.data) && prevModelData.data != 'null') {
            ...
            $("#cmx-i-2").val(data.item1); //现状勘察现状照片及现状图
            $("#cmx-i-3").val(data.item2); //保护原则
            $("#cmx-i-4").val(data.item3); //工程性质、范围、规模定位
            $("#cmx-i-5").val(data.item4); //保护措施、方案图纸
            $("#cmx-i-7").val(data.expertOpinion); //意见
            $('#cmx-i-8 input[value="' + data.expertResults + '"').attr("checked", true);
            $("#cmx-i-9").val(data.remark); //备注
