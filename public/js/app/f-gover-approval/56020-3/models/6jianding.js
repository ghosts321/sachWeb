...
cmx.route.model({
    ...
    handle: function (parameter, prevModelData, send, abort) {
        ...
        if (param.state == '200') {
            ...
            setTimeout(function () {
                window.location.href = public_url + 'app/f-gover-approval/56020-3/6jianding.html?nowid=' + GetUrlParamString('nowid');
            }, 1000)
        } else {
            showAlert({
                type: 'error',
        ...
        $(".editRelic").click(function () {
            ...
            $("#cmx-relic-modal").load(editRelicInfoModal, function () {
                ...
                $('#cmx-editrelicinfo').on('shown.bs.modal', function () {
                    ...
                    $('#cmx-30-IsRelic').parent().css('display','none');
                    // $('#cmx-30-IsAllow').parent().css('display','none');
