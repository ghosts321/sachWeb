...
cmx.route.model({
    ...
    handle: function (parameter, prevModelData, send, abort) {
        ...
        if (param.state == '200') {
            ...
            setTimeout(function () {
                window.location.href = public_url + 'app/f-gover-approval/56020-3/7shenpi.html?nowid=' + GetUrlParamString('nowid');
            }, 1000)
        } else {
            showAlert({
                type: 'error',
...
cmx.route.model({
    ...
    handle: function (parameter, prevModelData, send, abort) {
        var param = prevModelData;
        console.log(param);
        if (param.state == '200'&&!IsEmpty(param.data)) {
            if(param.data>0){
                showAlert({
                    type: 'confirm',
                ...
                getApplyList(3, '/eaScrApplyInfo/getApproveList', 8);
            }
