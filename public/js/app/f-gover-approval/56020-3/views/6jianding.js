    ...
    resolve: function (param) {
        ...
        if (data.length > 0) {
            for (var i = 0; i < data.length; i++) {
                ...
                '<button func-id="0101030102120100" data-id="' + applyId + '" style="margin:5px;" class="btn btn-primary btn-xs cmx-accept">提交</button><button func-id="0101030102120200" data-id="' + applyId + '"  style="margin:5px;" class="btn btn-primary btn-xs cmx-refuse">退回</button>',
...
cmx.route.view({
    ...
    resolve: function (param) {
        ...
        $("#relicList").empty();
        if(data){
            relicListJianding(data,pageInfo,3);
        }else {
            $("#relicTotal").text('共0条')
        }
