var get56019ProjectData = public_url + 'data/app/f-gover-approval/56019/56019-apply.json';
var _projectNum = '56019';
var _projectType = '4';
$(document).ready(function() {
    ...
    $("#cmx-button-save").on("click", function() {
        ...
        var formData = [{
            fileNumBack: $("#cmx-i-94-3").val(), //文件号最后一个 类型：String  必有字段  备注：无
            fileNumFront: $("#cmx-i-94-1").val(), //文件号第一个 类型：String  必有字段  备注：无
            fileNumMidd: $("#cmx-i-94-2").val(), //文件号第二个 类型：String  必有字段  备注：无
            ...
            samplingReson: $('#cmx-i-16').val(), //取样缘由
            specificStep: $('#cmx-i-17').val(), //具体工艺与步骤
            riskAssess: $('#cmx-i-18').val(), //风险评估与安全措施
        ...
        new cmx.process()
            .turn('callajax', {
                url: api_ea + '/eaMohMcpApply/saveprocessInstanceId',
                data: JSON.stringify({
                    token: getData('token'), //类型：String  必有字段  备注：无
                    // relicId:
                    mmaFormData: formData
                }),
                type: 'POST',
    ...
    $("#cmx-button-send").on("click", function() {
        ...
        new cmx.process()
            .turn('callajax', {
                url: api_ea + '/eaMohMcpApply/sendEaMohMcpApply',
                data: JSON.stringify({
                    token: getData('token'), //类型：String  必有字段  备注：无
                    mmaFormData: formData
                }),
    ...
    if (!IsEmpty(GetUrlParamString('applyId'))) {
        ...
        new cmx.process()
            .turn('callajax', {
                url: api_ea + '/eaMohMcpApply/getEntityByPKAndPNum',
                data: JSON.stringify({
                    token: getData('token'), //类型：String  必有字段  备注：无
...
cmx.route.model({
    ...
    handle: function(parameter, prevModelData, send, abort) {
        ...
        if (!IsNull(prevModelData) && prevModelData.state == '200' && !IsEmpty(prevModelData.data) && prevModelData.data != 'null') {
            ...
            $("#cmx-i-94-1").val(data.fileNumFront); //文件号前 
            //文件号第二个 
            ...
            $('#cmx-i-16').val(data.samplingReson); //取样缘由
            $('#cmx-i-17').val(data.specificStep); //具体工艺与步骤
            $('#cmx-i-18').val(data.riskAssess); //风险评估与安全措施
