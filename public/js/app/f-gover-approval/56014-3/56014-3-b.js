var get560143bNoNationProjectData = public_url + 'data/app/f-gover-approval/56014-3/56014-3-b.json';
var _projectNum = '56014-3_b';
var _projectType = '3';
...
$(document).ready(function() {
    ...
    $.ajax({
        url: get560143bNoNationProjectData,
        type: "GET",
        async: false,
...
function cmx_special_3(data) {
    var html = [
        ...
        data.columnindex + '" name="' + data.columnindex + '" placeholder="' + data.extra.placeholder +
        '" value=""></div>',
        '<div class="col-sm-3 col-md-3 col-lg-3"><span>（万元）</span></div>',
    ].join("");
    $("#cmx-special-" + data.serialnumber).append(html);
}
...
cmx.route.model({
    ...
    handle: function(parameter, prevModelData, send, abort) {
        ...
        if (!IsNull(prevModelData) && prevModelData.state == '200' && !IsEmpty(prevModelData.data) && prevModelData.data != 'null') {
            ...
            time = time.substring(0, 10).replace(/-/, "年");
            time = time.replace(/-/, "月");
            time = time + '日';
            console.log(time);
