var get560143eNoNationProjectData = public_url + 'data/app/f-gover-approval/56014-3/56014-3-e.json';
var _projectNum = '56014-3_e';
var _projectType = '3';
...
$(document).ready(function() {
    ...
    $.ajax({
        url: get560143eNoNationProjectData,
        type: "GET",
        async: false,
    ...
    $("#cmx-noNationProject-save").on("click", function() {
        ...
        new cmx.process()
            .turn('callajax', {
                url: api_ea + '/eaAecrApplyE/saveprocessInstanceId',
                data: JSON.stringify({
    ...
    $("#cmx-noNationProject-send").on("click", function() {
        ...
        new cmx.process()
            .turn('callajax', {
                url: api_ea + '/eaAecrApplyE/sendeaAecrApplyE',
                data: JSON.stringify({
                    token: getData('token'), //类型：String  必有字段  备注：无
                    // relicId:
                    ipaFormData: buildFormData
    ...
    if (!IsEmpty(GetUrlParamString('applyId'))) {
        ...
        new cmx.process()
            .turn('callajax', {
                url: api_ea + '/eaAecrApplyE/getEntityByPKAndPNum',
                data: JSON.stringify({
                    token: getData('token'), //类型：String  必有字段  备注：无
    ...
    handle: function(parameter, prevModelData, send, abort) {
        ...
        if (!IsNull(prevModelData) && prevModelData.state == '200' && !IsEmpty(prevModelData.data) && prevModelData.data != 'null') {
           ...
           // console.log(time);
            ...
            // $("#cmx-i-2-2 option[value='" + data.fileNumMidd + "']").attr("selected", true); //省文件号第二个 
            ...
            $("#cmx-i-7").val(data.projectName); //项目名称
