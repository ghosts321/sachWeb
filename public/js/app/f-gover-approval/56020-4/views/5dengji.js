    ...
    resolve: function (param) {
        ...
        if (data.length > 0) {
            for (var i = 0; i < data.length; i++) {
                ...
                '<button func-id="0102410400000000"  data-type="' + applyClassName + '" class="btn btn-info btn-xs exit-application-update" data-id="' + applyId + '">修改</button><button func-id="0101030103040200" data-id="' + applyId + '" style="margin:5px;" class="btn btn-primary btn-xs cmx-accept">提交</button><button func-id="0101030103040300" data-id="' + applyId + '"  style="margin:5px;" class="btn btn-primary btn-xs cmx-refuse">退回</button>',
                    ...
                    $('#cmx-carry-person-info').on('shown.bs.modal', function () {
                        ...
                        setTimeout(function () {
                            ...
                            $('#carrierApplication').on('click', (function () {
                                ...
                                new cmx.process()
                                    .turn('callajax', {
                                        url: api_ea + '/eaScrApplyInfo/insertTempComeApply',
                                        data: JSON.stringify({
                                            token: getData('token'),
                                            formData: formData
                                        })
...
cmx.route.view({
    ...
    resolve: function (param) {
        ...
        getApplyList(4, '/eaScrApplyInfo/getCurrenList', 5);
    },
    reject: function (data) {
        showAlert({
            type: 'error',
