var _projectType = '2';
var _projectNum = '56022_c';
if (getData('role') == 'guobaodanwei') {
    var get56022cNoNationProjectData = public_url + 'data/app/f-gover-approval/56022/56022-c-x.json';
} else {
    var get56022cNoNationProjectData = public_url + 'data/app/f-gover-approval/56022/56022-c-s.json';
}
$(document).ready(function () {
            ...
            .turn(function (prevModelData, send, abort) {
                if (prevModelData.state == '200') {
                    if (prevModelData.data.accClass == '15') {
                        get56022cNoNationProjectData = public_url + 'data/app/f-gover-approval/56022/56022-c-x.json';
                    }
                    else {
                        get56022cNoNationProjectData = public_url + 'data/app/f-gover-approval/56022/56022-c-s.json';
                    }
                } else {
                    showAlert({
                        type: 'error',
    ...
    CreateApplicationForm({
        element: '#cmx-form',
        url: get56022cNoNationProjectData
    });
    $('#cmx-menu')
        .jstree({
            'core': {
    ...
    $("#cmx-noNationProject-save").on("click", function () {
        ...
        //特殊验证
        if (!IsEmpty($('#cmx-i-4-1').val()) && $('#cmx-i-4-1').val().length > 20) {
            showAlert({
                type: "info",
        ...
        var re = /^[0-9]*$/;
        if (!IsEmpty(fileNumBack) && !re.test(fileNumBack)) {
            showAlert({
                type: "info",
        ...
        if (!IsEmpty(fileNumBack) && fileNumBack.length > 8) {
            showAlert({
                type: "info",
        ...
        var pro_title = $('#cmx-form #cmx-i-10').val(); //项目名称
        ...
        new cmx.process()
            .turn('callajax', {
                url: api_ea + '/eaIchProApplyC/saveprocessInstanceId',
                data: JSON.stringify({
    ...
    $("#cmx-noNationProject-send").on("click", function () {
        ...
        new cmx.process()
            .turn('callajax', {
                url: api_ea + '/eaIchProApplyC/sendEaIchProApplyC',
                data: JSON.stringify({
                    token: getData('token'), //类型：String  必有字段  备注：无
                    // relicId:
                    ipaFormData: ss
    ...
    if (!IsEmpty(GetUrlParamString('applyId'))) {
        new cmx.process()
            .turn('callajax', {
                url: api_ea + '/eaIchProApplyC/getEntityByPKAndPNum',
                data: JSON.stringify({
                    token: getData('token'), //类型：String  必有字段  备注：无
                    applyId: cmx.g.applyId, //类型：String  必有字段  备注：申请ID
                    projectNum: _projectNum //类型：String  必有字段  备注：项目编号
...
cmx.route.model({
    ...
    handle: function (parameter, prevModelData, send, abort) {
        ...
        if (!IsNull(prevModelData) && prevModelData.state == '200' && !IsEmpty(prevModelData.data) && prevModelData.data != 'null') {
            ...
            // $("#cmx-i-4-2 option[value='" + data.fileNumMidd + "']").attr("selected", "selected"); //省文件号中
