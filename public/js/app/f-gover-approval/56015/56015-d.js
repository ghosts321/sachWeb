/*
 * @Author: lvjinxiu 
 * @Date: 2017-12-11 16:18:44 
 * @Last Modified by: lvjinxiu
 * @Last Modified time: 2018-05-21 16:57:56
 */

var get56015cProjectData = public_url + 'data/app/f-gover-approval/56015/56015-d.json';
var get56015DetailModal = public_url + 'app/f-gover-approval/56015/include/56015-relic-detail-modal-2.html';
var get56015cDetailFormData = public_url + 'data/app/f-gover-approval/56015/relic-detail-d.json';
cmx.g.regist('proId', []);
var _projectType = '4';
var _projectNum = '56015_d';
$(document).ready(function () {
        ...
        new cmx.process()
            .turn('callajax', {
                ...
                data: JSON.stringify({
                    ...
                    projectNum: "56015_d" //类型：String  必有字段  备注：项目编号
                }),
                type: 'POST'
            })
            ...
            .turn(function (prevModelData, send, abort) {
                ...
                if (prevModelData.state == '200') {
                    ...
                    for (var i = 0; i < resultData.length; i++) {
                        ...
                        $("#cmx-rubbing-tbody").append(tr_html);
                        cmx.g.proId.push({
                            proId: resultData[i].proId
                        });
                    }
                    if (GetUrlParamString('from') == 'iframe') {
                        $('#cmx-rubbing-table thead tr').find('th').first().hide();
                        $("#cmx-rubbing-tbody tr").each(function () {
    ...
    $('#cmx-download-btn').on('click', function () {
        if (IsEmpty(cmx.g.applyId)) {
            ...
            window.open(api_ea + '/eaMohCopApply/exportExcel?token=' + getData('token') + '&applyId=' + cmx.g.applyId + '&projectNum=56015_d');
        }
    })
    //选择博物馆
    ...
    $("#cmx-select-delect-btn").on("click", function () {
        $('#cmx-rubbing-tbody input[type="checkbox"]').each(function () {
            if ($(this).is(":checked")) {
