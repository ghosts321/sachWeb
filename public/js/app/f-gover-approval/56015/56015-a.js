/*
 * @Author: lvjinxiu 
 * @Date: 2017-12-11 16:30:24 
 * @Last Modified by: lvjinxiu
 * @Last Modified time: 2018-05-21 16:54:00
 */

var get56015aProjectData = public_url + 'data/app/f-gover-approval/56015/56015-a.json';
var get56015DetailModal = public_url + 'app/f-gover-approval/56015/include/56015-relic-detail-modal.html';
var get56015aDetailFormData = public_url + 'data/app/f-gover-approval/56015/relic-detail-a.json';

cmx.g.regist('detailId', []);
$(document).ready(function () {
    $('#cmx-qingmaojun-back').on('click',function(){
        history.go(-1);
    });
    //加载表单
    cmx.g.applyId = "";
    //构建表单
    CreateApplicationForm({
        "url": get56015aProjectData,
        "element": "#cmx-form"
    });
    $("#cmx-form #cmx-i-7").attr('readonly', 'readonly');
    //获取待办列表信息详情
    if (!IsEmpty(GetUrlParamString('applyId'))) {
        cmx.g.applyId = GetUrlParamString('applyId'); //alert
        new cmx.process()
            .turn('callajax', {
                url: api_ea + '/eaMohRepApply/getEntityByPKAndPNum',
                data: JSON.stringify({
                    token: getData('token'), //类型：String  必有字段  备注：无
                    applyId: cmx.g.applyId, //类型：String  必有字段  备注：申请ID
                    projectNum: "56015_a" //类型：String  必有字段  备注：项目编号
                }),
                type: 'POST'
            })
            .turn(function (prevModelData, send, abort) {
                console.log(prevModelData);
                if (prevModelData.state == '200') {
                    var resultData = prevModelData.data;
                    $('#cmx-form #cmx-i-2-1').val(resultData.fileNumFront);
                    $('#cmx-form #cmx-i-2-2').val(resultData.fileNumMidd);
                    $('#cmx-form #cmx-i-2-3').val(resultData.fileNumBack);
                    $('#cmx-form #cmx-i-3').val(resultData.proFileTitle);
                    $('#cmx-form #cmx-i-4').val(resultData.contactName);
                    $('#cmx-form #cmx-i-5').val(resultData.contactTel);
                    $('#cmx-form #cmx-i-7').val(resultData.apprUnitName);
                    $('#cmx-form #cmx-i-8').val(resultData.apprContactName);
                    $('#cmx-form #cmx-i-9').val(resultData.apprContactTel);
                    cmx.g.museumId = resultData.apprUnit;
                    cmx.g.museumName = resultData.apprUnitName;
                    if (GetUrlParamString('from') == 'iframe') {
                        $('#cmx-form input').attr('disabled', true);
                        $('#cmx-form select').attr('disabled', true);
                        $('#cmx-form .cmx-museum-choose').hide();
                        $('#cmx-select-delect-btn').hide();
                        $('#cmx-select-relic-btn').hide();
                    }
                }
                send.go();
            })
            .turn('callajax', {
                url: api_ea + '/eaMohRepApply/getRelicdetailList',
                data: JSON.stringify({
                    token: getData('token'), //类型：String  必有字段  备注：无
                    applyId: cmx.g.applyId, //类型：String  必有字段  备注：申请ID
                    projectNum: "56015_a" //类型：String  必有字段  备注：项目编号
                }),
                type: 'POST'
            })
            .turn(function (prevModelData, send, abort) {
                console.log(prevModelData);
                if (prevModelData.state == '200') {
                    var resultData = prevModelData.data;
                    for (var i = 0; i < resultData.length; i++) {
                        var tr_html = '';
                        tr_html = ['<tr>',
                            '<td>',
                            '<input type="checkbox" name="cmx-handle-relic" value="' + resultData[i].detailId + '">',
                            '</td>',
                            '<td>' + resultData[i].sumMark + '</td>',
                            '<td>' + resultData[i].relicName + '</td>',
                            '<td>' + resultData[i].relicGrain + '</td>',
                            '<td>' + resultData[i].collectUnit + '</td>',
                            '<td>' + resultData[i].saveType + '</td>',
                            '<td>' + resultData[i].subsidyFunds + '</td>',
                            '<td><button onclick="editWenwuItem(\'' + resultData[i].detailId + '\')" class="btn btn-primary cmx-detail-btn">' + ((GetUrlParamString('from') == 'iframe') ? "查看详情" : "编辑文物详情") + '</button></td>',
                            '</tr>'
                        ].join('');
                        $("#cmx-restoration-plan-tbody").append(tr_html);
                        cmx.g.detailId.push({
                            detailId: resultData[i].detailId
                        });
                    }
                    if (GetUrlParamString('from') == 'iframe') {
                        $('#cmx-restoration-table thead tr').find('th').first().hide();
                        $("#cmx-restoration-plan-tbody tr").each(function () {
                            $(this).find('td').first().hide();
                        })
                    }
                }
            })
            .start();
    }
    //下载
    $('#cmx-download-btn').on('click', function () {
        if (IsEmpty(cmx.g.applyId)) {
            return;
        } else {
            window.open(api_ea + '/eaMohRepApply/exportExcel?token=' + getData('token') + '&applyId=' + cmx.g.applyId + '&projectNum=56015_a');
        }
    })
    //选择博物馆
    $(".cmx-museum-choose").on("click", function () {
        new cmx.process()
            .turn('buildMuseumInfoModal', {
                goto: function (type) {
                    $("#cmx-form #cmx-i-7").val(cmx.g.museumName);
                    $("#cmx-museum-modal").modal('hide');
                }
            })
            .ccatch(function (msg) {})
            .cfinally(function () {}).start();
    });
    //选择文物模态框
    $("#cmx-select-relic-btn").on("click", function () {
        if (IsEmpty(cmx.g.applyId)) {
            showAlert({
                type: 'error',
                content: '请先保存申请信息'
            });
            return;
        }
        new cmx.process()
            .turn('buildRelicInfoModal', {
                callback: function (param) {
                    $("#cmx-detail-modal-div").load(get56015DetailModal, function () {
                        $("#cmx-relic-detail-modal").off('shown.bs.modal');
                        $("#cmx-relic-detail-modal").on('shown.bs.modal', function () {
                            /* 文物古籍数据展开查询条件 */
                            $('#cmx-detail-form').html('');
                            //构建表单
                            CreateApplicationForm({
                                "url": get56015aDetailFormData,
                                "element": "#cmx-detail-form"
                            });
                            var data = {
                                "projectNum": "56015_a",
                                "detailId": "",
                                "collectUnit": param.museumId,
                                "relicName": param.antiqueName,
                                "sumMark": param.relicCode,
                                "relicGrain": param.relicQualityName,
                                "saveType": param.saveStateName,
                                "valueAssess": "",
                                "saveState": "",
                                "mainTarget": "",
                                "proEstimates": "",
                                "subsidyFunds": "",
                                "apprOpinion": "",
                                "opinionDes": "",
                                "remark": ""
                            }
                            saveAndAddRecilData(data);
                        });
                        $("#cmx-relic-detail-modal").modal('show');
                    });
                }
            })
            .ccatch(function (msg) {}).cfinally(function () {}).start();
    });
    //移除
    $("#cmx-select-delect-btn").on("click", function () {
        $('#cmx-restoration-plan-tbody input[type="checkbox"]').each(function () {
            if ($(this).is(":checked")) {
                $(this).parent().parent().remove();
                for (var i = 0; i < cmx.g.detailId.length; i++) {
                    if (cmx.g.detailId[i].detailId == $(this).val()) {
                        cmx.g.detailId.splice(i, 1);
                    }
                }
            }
        });
    });
    $('#cmx-menu')
        .jstree({
            'core': {
                'multiple': false,
                'data': [{
                    "text": "可移动文物修复项目计划",
                    "state": {
                        "opened": true
                    },
                    "children": [{
                        "id": 1,
                        "text": "基本信息",
                        "state": {
                            "selected": true
                        }
                    }]
                }]
            }
        }).on("loaded.jstree", function (e, datas) {
            //初始化后事件
        }).on("changed.jstree", function (e, datas) {

        });
    //保存
    $("#cmx-noNationProject-save").on("click", function () {
        //基本表单验证
        var temp_flag = checkFormLength('#cmx-form');
        if (!temp_flag) {
            return;
        }
        if (!IsEmpty($('#cmx-i-2-1').val())) {
            if ($('#cmx-i-2-1').val().length > 20) {
                showAlert({
                    type: "info",
                    content: "省文件号前可填长度最大为20"
                });
                return;
            }
        }
        var fileNumBack = $('#cmx-form #cmx-i-2-3').val();
        var re = /^[0-9]*$/;
        if (!IsEmpty(fileNumBack)) {
            if (!re.test(fileNumBack)) {
                showAlert({
                    type: "info",
                    content: "省文件号后可填项只能填数字"
                });
                return;
            }
            if (fileNumBack.length > 8) {
                showAlert({
                    type: "info",
                    content: "省文件号后可填长度最大为8"
                });
                return;
            }
        }
        if (IsEmpty(cmx.g.museumName)) {
            showAlert({
                type: "info",
                content: "请选择博物馆名称"
            });
            return;
        }

        var formate = [{
            "applyId": cmx.g.applyId,
            "projectNum": "56015_a",
            "projectType": 4,
            "fileNumFront": $('#cmx-form #cmx-i-2-1').val(),
            "fileNumMidd": $('#cmx-form #cmx-i-2-2').find('option:selected').val(),
            "fileNumBack": $('#cmx-form #cmx-i-2-3').val(),
            "contactName": $('#cmx-form #cmx-i-4').val(),
            "contactTel": $('#cmx-form #cmx-i-5').val(),
            "relicDetailList": cmx.g.detailId,
            "apprContactName": $('#cmx-form #cmx-i-8').val(),
            "apprContactTel": $('#cmx-form #cmx-i-9').val(),
            "proFileTitle": $('#cmx-form #cmx-i-3').val(),
            "apprUnitName": cmx.g.museumName,
            "apprUnit": cmx.g.museumId,
            "publishType": 1,
            projectName: "馆藏一级文物的修复计划"
        }];
        console.log(formate);
        new cmx.process()
            .turn('callajax', {
                url: api_ea + '/eaMohRepApply/saveprocessInstanceId',
                data: JSON.stringify({
                    token: getData('token'), //类型：String  必有字段  备注：无
                    mraFormData: formate
                }),
                type: 'POST'
            })
            .turn(function (prevModelData, send, abort) {
                console.log(prevModelData);
                if (prevModelData.state == '200') {
                    showAlert({
                        type: "success",
                        content: "保存成功"
                    })
                    cmx.g.applyId = prevModelData.data.applyId;
                }
            })
            .start();
    });
    //发送
    $("#cmx-noNationProject-send").on("click", function () {
        //基本表单验证
        var temp_flag = checkFormLength('#cmx-form');
        if (!temp_flag) {
            return;
        }
        //特殊验证
        if (!IsEmpty($('#cmx-i-2-1').val())) {
            if ($('#cmx-i-2-1').val().length > 20) {
                showAlert({
                    type: "info",
                    content: "省文件号前可填长度最大为20"
                });
                return;
            }
        }
        var fileNumBack = $('#cmx-form #cmx-i-2-3').val();
        var re = /^[0-9]*$/;
        if (!IsEmpty(fileNumBack)) {
            if (!re.test(fileNumBack)) {
                showAlert({
                    type: "info",
                    content: "省文件号后可填项只能填数字"
                });
                return;
            }
            if (fileNumBack.length > 8) {
                showAlert({
                    type: "info",
                    content: "省文件号后可填长度最大为8"
                });
                return;
            }
        }
        if (IsEmpty(cmx.g.museumName)) {
            showAlert({
                type: "info",
                content: "请选择博物馆名称"
            });
            return;
        }

        var formate = [{
            "applyId": cmx.g.applyId,
            "projectNum": "56015_a",
            "projectType": 4,
            "fileNumFront": $('#cmx-form #cmx-i-2-1').val(),
            "fileNumMidd": $('#cmx-form #cmx-i-2-2').find('option:selected').val(),
            "fileNumBack": $('#cmx-form #cmx-i-2-3').val(),
            "contactName": $('#cmx-form #cmx-i-4').val(),
            "contactTel": $('#cmx-form #cmx-i-5').val(),
            "relicDetailList": cmx.g.detailId,
            "apprContactName": $('#cmx-form #cmx-i-8').val(),
            "apprContactTel": $('#cmx-form #cmx-i-9').val(),
            "proFileTitle": $('#cmx-form #cmx-i-3').val(),
            "apprUnitName": cmx.g.museumName,
            "apprUnit": cmx.g.museumId,
            "publishType": 1,
            projectName: "馆藏一级文物的修复计划"
        }];
        console.log(formate);
        new cmx.process()
            .turn('callajax', {
                url: api_ea + '/eaMohRepApply/sendEaMohRepApply',
                data: JSON.stringify({
                    token: getData('token'), //类型：String  必有字段  备注：无
                    mraFormData: formate
                }),
                type: 'POST'
            })
            .turn(function (prevModelData, send, abort) {
                console.log(prevModelData);
                if (prevModelData.state == '200') {
                    showAlert({
                        type: "success",
                        content: "发送成功"
                    });
                    setTimeout(function () {
                        window.location.href = '/app/f-gover-approval/counterpart/counterpart-needToDo.html?type=1&nowid='+GetUrlParamString('nowid');
                    }, 1000);
                }
                send.go();
            })
            // .turn('callajax', {
            //     url: api_ea + '/eaPubTransactionrecord/disposeRecord',
            //     data: JSON.stringify({
            //         token: getData('token'), //类型：String  必有字段  备注：无
            //         trsFlag: "1",
            //         trsFormData: [{
            //             applyId: cmx.g.applyId,
            //             projectNum: "56015_a",
            //             receiverId: "RGJ0101002",
            //             note: "",
            //             recordId: ""
            //         }]
            //     }),
            //     type: 'POST',
            //     success: function (result) {
            //         console.log(result);
            //         if (result.state == '200') {
            //             setTimeout(function () {
            //                 window.location.href = '/app/f-gover-approval/counterpart/counterpart-needToDo.html?nowid='+GetUrlParamString('nowid');
            //             }, 1000);
            //         }
            //     }
            // })
            .start();
    });
});

function editWenwuItem(detailId) {
    new cmx.process()
        .turn('callajax', {
            url: api_ea + '/eaMohRepApply/getRepRelicdetail',
            data: JSON.stringify({
                token: getData('token'), //类型：String  必有字段  备注：无
                detailId: detailId
            }),
            type: 'POST'
        })
        .turn(function (prevModelData, send, abort) {
            console.log(prevModelData);
            if (prevModelData.state == '200') {
                var data = prevModelData.data;
                $("#cmx-detail-modal-div").load(get56015DetailModal, function () {
                    $("#cmx-relic-detail-modal").off('shown.bs.modal');
                    $("#cmx-relic-detail-modal").on('shown.bs.modal', function () {
                        /* 文物古籍数据展开查询条件 */
                        $('#cmx-detail-form').html('');
                        //构建表单
                        CreateApplicationForm({
                            "url": get56015aDetailFormData,
                            "element": "#cmx-detail-form"
                        });
                        saveAndAddRecilData(data);
                    });
                    $("#cmx-relic-detail-modal").modal('show');
                });
            }
        })
        .start();
}

//编辑新增文物
function saveAndAddRecilData(data) {
    console.log(data);
    $("#cmx-detail-form #cmx-i-1").val(data.collectUnit).attr('readonly', 'readonly');
    $("#cmx-detail-form #cmx-i-2").val(data.relicName).attr('readonly', 'readonly');
    $("#cmx-detail-form #cmx-i-3").val(data.sumMark).attr('readonly', 'readonly');
    $("#cmx-detail-form #cmx-i-4").val(data.saveType).attr('readonly', 'readonly');
    $("#cmx-detail-form #cmx-i-5").val(data.relicGrain).attr('readonly', 'readonly');
    $("#cmx-detail-form #cmx-i-6").val(data.valueAssess);
    $("#cmx-detail-form #cmx-i-7").val(data.saveState);
    $("#cmx-detail-form #cmx-i-8").val(data.mainTarget);
    $("#cmx-detail-form #cmx-i-9").val(data.proEstimates);
    $("#cmx-detail-form #cmx-i-10").val(data.subsidyFunds);

    if (GetUrlParamString('from') == 'iframe') {
        $("#cmx-detail-form textarea").attr('readonly', 'readonly');
        $("#cmx-detail-form input").attr('readonly', 'readonly');
        $('.form-file').hide();
        $('#cmx-relic-detail-save').hide();
    }
    if (GetUrlParamString('from') != 'iframe') {
        //编辑文物保存
        $('#cmx-relic-detail-save').on('click', function () {
            //基本表单验证
            var temp_flag = checkFormLength('#cmx-detail-form');
            if (!temp_flag) {
                return;
            }
            if (!IsEmpty($("#cmx-detail-form #cmx-i-10").val())) {
                var num = $("#cmx-detail-form #cmx-i-10").val();
                num = num.replace('.', '');
                if (num.length > 16) {
                    showAlert({
                        type: "info",
                        content: "拟申请补助经费可填最大长度为16(保留两位小数)"
                    });
                    return;
                }
            }
            var mrrFormData = [{
                "projectNum": "56015_a",
                "detailId": data.detailId,
                "applyId": cmx.g.applyId,
                "collectUnit": data.collectUnit,
                "relicName": data.relicName,
                "sumMark": data.sumMark,
                "relicGrain": data.relicGrain,
                "saveType": data.saveType,
                "valueAssess": $('#cmx-detail-form #cmx-i-6').val(),
                "saveState": $('#cmx-detail-form #cmx-i-7').val(),
                "mainTarget": $('#cmx-detail-form #cmx-i-8').val(),
                "proEstimates": $('#cmx-detail-form #cmx-i-9').val(),
                "subsidyFunds": $('#cmx-detail-form #cmx-i-10').val(),
                "apprOpinion": "",
                "opinionDes": "",
                "remark": ""
            }];
            new cmx.process()
                .turn('callajax', {
                    url: api_ea + '/eaMohRepApply/saveEaMohRepRelicdetail',
                    data: JSON.stringify({
                        token: getData('token'), //类型：String  必有字段  备注：无
                        mrrFormData: mrrFormData
                    }),
                    type: 'POST'
                })
                .turn(function (prevModelData, send, abort) {
                    console.log(prevModelData);
                    if (prevModelData.state == '200') {
                        showAlert({
                            type: "success",
                            content: "保存成功"
                        });
                        var flag = true;
                        for (var i = 0; i < cmx.g.detailId.length; i++) {
                            if (cmx.g.detailId[i].detailId == prevModelData.data.detailId) {
                                flag = false;
                            }
                        }
                        if (flag) {
                            cmx.g.detailId.push({
                                detailId: prevModelData.data.detailId,
                            });
                        }
                        console.log(cmx.g.detailId)
                        $('#cmx-restoration-plan-tbody input[type="checkbox"]').each(function () {
                            if ($(this).val() == prevModelData.data.detailId) {
                                $(this).parent().parent().remove();
                            }
                        })
                        var tr_html = '';
                        tr_html = ['<tr>',
                            '<td>',
                            '<input type="checkbox" name="cmx-handle-relic" value="' + prevModelData.data.detailId + '">',
                            '</td>',
                            '<td>' + prevModelData.data.sumMark + '</td>',
                            '<td>' + prevModelData.data.relicName + '</td>',
                            '<td>' + prevModelData.data.relicGrain + '</td>',
                            '<td>' + prevModelData.data.collectUnit + '</td>',
                            '<td>' + prevModelData.data.saveType + '</td>',
                            '<td>' + prevModelData.data.subsidyFunds + '</td>',
                            '<td><button onclick="editWenwuItem(\'' + prevModelData.data.detailId + '\')" class="btn btn-primary cmx-detail-btn">编辑文物详情</button></td>',
                            '</tr>'
                        ].join('');
                        $("#cmx-restoration-plan-tbody").append(tr_html);
                        $('#cmx-relic-detail-modal').modal("hide");
                    }
                })
                .start();
        })
    }
}

function cmx_special_2(data) {
    var html = [
        '<div class="col-sm-9 col-md-9 col-lg-9 cmx-special-provincefile">',
        '<input type="text" class="form-control" id="' +
        data.columnindex + '" name="' + data.columnindex + '" placeholder="' + data.extra.placeholder +
        '" value=""></div>',
        '<div class="col-sm-3 col-md-3 col-lg-3"><button class="cmx-museum-choose btn btn-primary">选择</button></div>',
    ].join("");
    $("#cmx-special-" + data.serialnumber).append(html);
}

function cmx_special_4(data) {
    var html = [
        '<div class="col-sm-9 col-md-9 col-lg-9 cmx-special-provincefile"><input type="text" class="form-control" id="' +
        data.columnindex + '" name="' + data.columnindex + '" placeholder="' + data.extra.placeholder +
        '" value=""></div>',
        '<div class="col-sm-3 col-md-3 col-lg-3"><button class="cmx-unitList-choose btn btn-primary">选择</button></div>',
    ].join("");
    $("#cmx-special-" + data.serialnumber).append(html);
}