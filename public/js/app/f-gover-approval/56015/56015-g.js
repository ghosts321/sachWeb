var get56015gProjectData = public_url + 'data/app/f-gover-approval/56015/56015-g.json';
var _projectNum = '56015_g';
var _projectType = '4';
$(document).ready(function() {
    ...
    $("#cmx-button-save").on("click", function() {
        ...
        var formData = [{
            ...
            mainDisease: $('#cmx-i-15').val(), //主要病害
            mainPurpose: $('#cmx-i-16').val(), //主要目标
        ...
        new cmx.process()
            .turn('callajax', {
                url: api_ea + '/eaMohRcrApplyG/saveprocessInstanceId',
                data: JSON.stringify({
    ...
    $("#cmx-button-send").on("click", function() {
        ...
        new cmx.process()
            .turn('callajax', {
                url: api_ea + '/eaMohRcrApplyG/sendEaMohRcrApplyG',
                data: JSON.stringify({
                    token: getData('token'), //类型：String  必有字段  备注：无
                    ipaFormData: formData
                }),
    ...
    if (!IsEmpty(GetUrlParamString('applyId'))) {
        ...
        new cmx.process()
            .turn('callajax', {
                url: api_ea + '/eaMohRcrApplyG/getEntityByPKAndPNum',
                data: JSON.stringify({
                    token: getData('token'), //类型：String  必有字段  备注：无
...
cmx.route.model({
    ...
    handle: function(parameter, prevModelData, send, abort) {
        ...
        if (!IsNull(prevModelData) && prevModelData.state == '200' && !IsEmpty(prevModelData.data) && prevModelData.data != 'null') {
            ...
            $('#cmx-i-15').val(data.mainDisease); //主要病害
            $('#cmx-i-16').val(data.mainPurpose); //主要目标
