    ...
    resolve: function (param) {
        ...
        if (data.length > 0) {
            for (var i = 0; i < data.length; i++) {
                ...
                '<button func-id="0101030101020100" data-id="' + applyId + '" class="btn btn-primary btn-xs cmx-accept">受理</button><button func-id="0101030101020200" data-id="' + applyId + '" class="btn btn-primary btn-xs cmx-yicibuzheng">一次性补正</button><button func-id="0101030101020300" data-id="' + applyId + '" class="btn btn-primary btn-xs cmx-bushouli">不予受理</button><button func-id="0101030101020400" data-id="' + applyId + '" class="btn btn-primary btn-xs cmx-updateYuyueTime">更改预约时间</button>',
                            ...
                            new cmx.process()
                                ...
                                .turn(function (prevModelData, send, abort) {
                                    if (prevModelData.state == "200" && !IsEmpty(prevModelData)) {
                                        ...
                                        getApplyList(1, '/eaScrApplyInfo/acceptApllyInfoList', 3);
                                    } else {
                                        showAlert({
                                            type: 'error',
                                            content: prevModelData.msg
...
cmx.route.view({
    ...
    resolve: function (param) {
        ...
        setTimeout(function () {
            window.location.href = public_url + 'app/f-gover-approval/56020-1/3shouli.html?nowid=' + GetUrlParamString('nowid');
        }, 1000)
    },
    reject: function (data) {
        showAlert({
