cmx.route.view({
    ...
    resolve: function (param) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            '<button func-id="0101030101060100" data-id="' + applyId + '"  class="btn btn-primary btn-xs cmx-shenpi">审批</button><button func-id="0101030101060200" data-id="' + applyId + '"  style="margin:5px;" class="btn btn-primary btn-xs exit-application-print" ' + isApprove + '>打印</button>',
            '<button func-id="0101030101060300" data-id="' + applyId + '"  class="btn btn-primary btn-xs cmx-accept" ' + isApprove + '>办结</button><button func-id="0101030101060400" data-id="' + applyId + '"  style="margin:5px;" class="btn btn-primary btn-xs cmx-refuse">退回</button>',
        ...
        $(".cmx-shenpi").on("click", function () {
            ...
            new cmx.process()
                .turn('callajax', {
                    url: api_ea + '/eaScrRelicInfo/getRelicType',
                    data: JSON.stringify({
                        token: getData('token'),
                        applyId: applyId
                    })
                ...
                .turn(function (prevModelData, send, abort) {
                    if (prevModelData.state == "200" && !IsEmpty(prevModelData.data)) {
                        showAlert({
                            type: 'confirm',//success info warning error confirm input
                            content: '是否是文物鉴定结果：<br>文物' + prevModelData.data.relicList + '件，复仿制品' + prevModelData.data.repList + '件。鉴定结果未统一' + prevModelData.data.relicRepList + '件。<br><br>是否允许出境鉴定结果：<br>允许出境' + prevModelData.data.allowList + '件，禁止出境' + prevModelData.data.banList + '件。鉴定结果未统一' + prevModelData.data.banAllowList + '件。',
                            btn_1: '返回',//可选参数，type为confirm时默认为确定，type为input时默认为提交
                            btn_2: '确定',//可选参数，默认为取消
                            callback: function (_state) {//仅type为confirm下有效
                                // console.log(_state);//_state可能是yes no cancel
                                if (_state == 'yes') {
