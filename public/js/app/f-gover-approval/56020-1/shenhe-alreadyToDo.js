var getPersonalAlreadyDetailsData = public_url + 'data/app/f-gover-approval/56020-1/getPersonalAlreadyDetails.json';
var getExhibitionAlreadyDetailsData = public_url + 'data/app/f-gover-approval/56020-1/getExhibitionAlreadyDetails.json';
...
$(document).ready(function () {
    ...
    $("#search-btn").on("click", function () {
        ...
        param.appUserName = $("#appUserName").val();
        ...
        .turn(function (prevModelData, send, abort) {
            if (!IsNull(prevModelData.data) && prevModelData.state == '200') {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    var acceptNum = IsEmpty(data[i].acceptNum) ? '' : data[i].acceptNum;
                    var applyStatus = data[i].applyStatus;
                    var reserveDate = IsEmpty(data[i].reserveDate) ? "" : format2(data[i].reserveDate);
                    var resDatePar = IsEmpty(data[i].resDatePar) ? "" : data[i].resDatePar;
                    var node = {
                        "2": "预约",
                        "3": "受理",
                        "4": "分办",
                    ...
                    var button = showPointButton(applyId,data[i].applyStatus);
                    if (data[i].cardType) {
                        for (var j = 0; j < data[i].cardType.length; j++) {
                            button += '<button func-id="0101030201000000" class="btn btn-success finsh-button btn-xs" data-id="' + applyId + '">' + data[i].cardType[j] + '</button>'
                        }
                    }
                ...
                if (applyClass == 1) {
                    $('#cmx-getDetails-modal').load(getDetailsModal, function () {
                        ...
                        $('#getDetails-modal').on('shown.bs.modal', function () {
                            getDetails(getExhibitionAlreadyDetailsData, applyId);
                            new cmx.process()
                                .turn('callajax', {
                    ...
                    $('#cmx-getDetails-modal').load(getDetailsModal, function () {
                        ...
                        $('#getDetails-modal').off('shown.bs.modal');
                        $('#getDetails-modal').on('shown.bs.modal', function () {
                            getDetails(getPersonalAlreadyDetailsData, applyId);
                            new cmx.process()
                                .turn('callajax', {
                                    url: api_ea + '/eaScrApplyInfo/getApplyInfoByApplyId',
                                    data: JSON.stringify({
