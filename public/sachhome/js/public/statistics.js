$(function () {
    ...
    var column = getQuery(window.location.href).column; // 从上一个页面获取传过来的参数column
    var category = getQuery(window.location.href).category;
    // var columnName;
    // if (category == '政府信息公开规定'){
    //     columnName = 
    // }
    // var obj = {
    //     '0500000000': '统计分析',
    ...
    // for (var i in obj) {
    //     if (i === column) {
    //         var columnName = obj[i]; // 找到这个column对应的分类
    //         break;
    //     }
    // }
    $('#info').html('>> ' + category); // 设置为面包屑导航
    ...
    // 封装一下
    function ajaxInfo(pagenum, pagesize, column, category, keyword, postdate) {
        $.ajax({
            data: { column: column, pagenum: pagenum, pagesize: 20, keyword: keyword, postdate: postdate },
            beforeSend: function () {
    ...
    $('#retrieve').on('click', function (e) {
        ...
        ajaxInfo(1, 20, column, category, keyword, postdate);
        $.ajax({
            data: { column: column, pagenum: 1, pagesize: 20, keyword: keyword, postdate: postdate },
            success: function (res) {
                ...
                if (res.data.dataList.length) {
                    ...
                    $('.M-box4').pagination({
                        ...
                        callback: function (api) {
                            ...
                            ajaxInfo(pagenum, 20, column, category, keyword, postdate);
                        }
                    });
                } else {
                    $('#sheet-tbody').html(msg.noData); // 没有数据显示暂无数据
