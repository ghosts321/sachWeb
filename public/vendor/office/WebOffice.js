function WebOffice2015() {
    ...
    this.WebOpen = function (mBoolean) {
        ...
        this.WebSetMsgByName("DOCCLASS", this.DocClass); //加载RecordID
    ...
    this.WebSave = function () {
        ...
        this.WebSetMsgByName("DOCCLASS", _DocClass);
        this.WebSetMsgByName("FILENAME", this.FileName); //加载FileName
        this.WebSetMsgByName("FILETYPE", this.FileType); //加载FileType
        this.WebSetMsgByName("DIRECTORYID", '50'); //加载FileType//50代表起文
    ...
    this.SAVEFILE = function (httpclient, FileName) { //时发生交互OPTION值为：SAVEFILE
        ...
        //alert(this.WebSaveUrl);
        if (httpclient.Open(this.HttpMethod.Post, this.WebSaveUrl, false)) { //这里采用同步方式打开文档。我要返回值
            if (!httpclient.Send()) {
                ...
                var _ResponseText = httpclient.ResponseText;
                var result_obj = eval('(' + _ResponseText + ')');
                if (result_obj.state == '200') {
                    this.Status = "保存文件成功";
                    _RecordID = result_obj.data[0].fileIndex;
                    this.RecordID = _RecordID;
                    //alert('走你'+this.RecordID);
                    return true;
                } else {
    ...
    this.LOADFILE = function (httpclient) { //Http 下载文档
        ...
        if (httpclient.Open(this.HttpMethod.Post, this.WebLoadUrl, false)) { //这里采用同步方式打开文档。我要返回值
            if (httpclient.Send()) {
                if (httpclient.GetResponseHeader("MsgError") == "404") { //判断服务器是否存在文件
                    // this.CreateFile();
                    // this.getOfficeVersion(); //打开文档后，判断当前office版本
                    httpclient.Clear();
                    var windows = window.confirm("文件未找到！\r\r单击“确定”关闭。单击“取消”继续。");
                    if (windows == 1) {
                        window.close();
                    }
                    return false;
    ...
    //下载文档
    this.DownloadToFile = function (DownFileName, SavePath, RecordID, DocClass) {
        var httpclient = this.obj.Http; //设置http对象
        httpclient.Clear();
        this.WebSetMsgByName("RECORDID", RecordID); //加载RecordID
        this.WebSetMsgByName("FILENAME", DownFileName); //加载FileName
        this.WebSetMsgByName("FILETYPE", this.FileType); //加载FileType
        this.WebSetMsgByName("DOCCLASS", DocClass); //加载RecordID
        this.WebSetMsgByName("OPTION", "LOADFILE"); //发送请求LOADFILE
        httpclient.AddForm("FormData", this.WebSendMessage()); //这里是自定义json 传输格式。
        this.WebClearMessage(); //清除所有WebSetMsgByName参数
        if (httpclient.Open(this.HttpMethod.Post, this.WebLoadUrl, false)) { //这里采用同步方式打开文档。我要返回值
    ...
    */
    ...
    this.VBAInsertFile = function (Position, FileName, func) {
        L('5');
        try {
            L('6');
    ...
    this.WebAcceptAllRevisions = function () {
        ...
        try {
            ...
            L('10' + this.obj.ActiveDocument.Revisions.Count >= 0);
